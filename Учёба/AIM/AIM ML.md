...
### [[AIM ML#AIM ML Content Решающие деревья|Решающие деревья]]
...
### [[AIM ML#Content|Деформация. Пороговые функции]]
...
Как вариант, можно сравнивать признак со множеством порогов $\{\theta_i\}$ и задаваться задачей аппроксимировать целевую функцию линейной комбинацией таких вот порогов $I(x < \theta_i)$. Тогда результат получится кусочно-константной функцией.

![[Pasted image 20230616200348.png]]

 Если хочется другой вид функции, можно домножать индикатор на значение признака — тогда она будет кусочно-линейной, пусть и с разрывами

![[Pasted image 20230616200425.png]]

А давайте уберем и разрывы! Деформация вида $$
\begin{cases}
	|x - \theta_i|^k, x \geq \theta_i\\0, x < \theta_i
\end{cases}$$
Превращает нашу функцию в гладкую аппроксимирующую кривую

![[Pasted image 20230616200659.png]]

&nbsp;

### [[AIM ML#Content|Деформация. Радиальные функции]]
Теперь давайте рассмотрим функции от расстояния до некоторой точки $A$. Назовём их *радиальными*. 
$$\varphi_A(x) = f(\|x - A\|):\;\mathbb{R}^n \rightarrow \mathbb{R}$$
Назовём радиальную функцию *базисной*, если для любого набора точек $A_1, A_2, \dots, A_n$ функции $\varphi_{A_1}(x), \varphi_{A_2}(x), \dots, \varphi_{A_3}(x)$ линейно независимы и матрица $a_{ij} = \varphi_{A_i}(A_j)$ невырождена

>[!example] Примеры радиально-базисных функций
>- $exp(-kr^2)$
>- $\sqrt{1+kr^2}$
>- $\dfrac{1}{1+kr^2}$
>- $r^2\ln r$
...
Методы, использующие в своей основе радиально-базисные функции, называются *RBF* (*Radial basis function*) алгоритмами. 

&nbsp;

### [[AIM ML#Content|Ядерные методы (Kernel tricks)]]

Идея у ядер всё та же — давайте брать нерешаемое линейными методами и деформировать так, чтобы стало решаемо. Однако до этого момента мы говорили про какие-то конкретные функции, добавляющие новые признаки

Теперь давайте подумаем, как этот процесс автоматизировать. Для начала вспомним SVM — там мы перешли к двойственной задачи. Конкретно там максимизировалось значение
$$\sum\limits_{i=1}^{n} \alpha_i - \dfrac{1}{2}\sum\limits_{j=1}^{n}\sum\limits_{i=1}^{n}\alpha_i\alpha_jy_iy_j{\color{goldenrod}x_ix_j}$$Зависившее **только** от попарных произведений значений признаков. То есть метод отлично работает и если мы забудем об исходных признаках и будем работать только с их произведениями. 

Давайте попробуем взять два вектора $x = (x_1, x_2, \dots, x_n)$ и $y = (y_1, y_2, \dots, y_n)$ и возвести в квадрат их скалярное произведение

$$x^Ty = (x_1y_1 + x_2y_2 + \dots + x_ny_n)^2 = $$
$$= x_1^2y_1^2 + x_1^2y_2^2 + \dots + x_n^2y_n^2 + \sum\limits_{i,j}\sqrt{2}x_ix_j\cdot\sqrt{2}y_iy_j\;\;(*)$$
Если мы введем обозначение $f(x) = (x_1^2, x_2^2, \dots, \sqrt{2}x_1x_2, \sqrt{2}x_1x_3, \dots)$, то выражение $(*)$ можно записать в виде $$(*) = f(x)^Tf(y)$$
Ну и что? А то, что решая тот же SVM, в котором максимизируется сумма скалярных произведений, и заменив эти скалярные произведения на их квадраты, мы также начнем решать задачу оптимизацию, но перейдем к новому признаковому пространству, в котором есть **все** мономы от наших признаков (а это может превратить пространство в линейно разделимое)

- - -

Наконец дадим определение. *Ядро* — это такая функция $K: X\times X \rightarrow \mathbb{R}$, что существует функция $\varphi:X \rightarrow F$ (*Гильбертово пространство*), такая что $$K(x,y) = \varphi(x)^T\varphi(x)$$
То есть это некоторое обобщение для квадрата скалярного произведения из выкладок выше (можем заменять скалярные произведения в SVM не только на $(x_i, x_j)^2$, но и на любое ядро $K(x_i, x_j)$)

Метод называется *кернализованным*, если в нём используются только попарные произведения, а стало быть в нём мы можем заменить эти произведения на ядро.

>[!quote]- Теорема Мерсера
>Если $K$ — неотрицательно определенное ядро, которое симметрично и для любой обучающей выборки *матрица ядра* неотрицательно определена, т.е. $$\forall w \in \mathbb{R}^n: w^T\|K(x_i,x_j)\|w \geq 0$$
>Тогда существует Гильбертово пространство, в котором $K$ — скалярное произведение

>[!example] Примеры ядер
>- $K(x,y) = (x^Ty)^k$ (случай с $k=2$ мы рассматривали ранее)
>- $K(x,y) = (x^Ty + 1)^k$ (ничуть не сложнее, но сохраняет в признаковом пространстве $x_1, x_2, \dots$)
>- $K(x,y) = \dfrac{1}{1-k^2x^Ty}$
>- $K(x,y) = \exp(-\gamma\|x-y\|)^k$

Ядра замкнуты относительно суммы, произведения и умножения на положительное число. Более того, сумма ядер отвечает за *конкатенацию* признаков, получаемых первым и вторым ядром.


&nbsp;

## [[AIM ML#Content|Решающие деревья]]

Один из первых разбираемых методов, помимо метрических, который действительно нелинеен по своей сути. Идея алгоритма проста (и похожа на то, как мы принимаем решения в реальной жизни) — каждая вершина дерева представляет собой *предикат* относительно признака, спускаясь по которому мы выдаём то или иное решение.

Решающие деревья решают как задачи классификации, так и регрессии (в этом случае в терминальных вершинах находятся регрессионные метки)

>[!example] Виды предикатов
Формально предикат в вершине дерева может быть любым. Выделим некоторые виды:
>
>- *Пороговый* для вещественного признака: $\mathbb{I}(f(x) \leq \theta)$
>- *Принадлежность категории* для категориального признака: $\mathbb{I}(f(x) \in C)$
>- *Косые деревья* (линейная комбинация как предикат: $\mathbb{I}(\sum w_if(x) \leq \theta)$
>- *Сферические деревья*: $\mathbb{I}(\sum (f(x) - z)^2 \leq \theta)$

Оптимизировать деревья в явном виде очень трудно. Поэтому для улучшения их качества используются не методы оптимизации, а эвристики.

&nbsp;

### [[AIM ML#Content|Какие значение предсказывать?]]

Допустим, мы построили дерево. *Какие значения нам предсказывать в его листьях*? Для начала заметим, что каждый листок соответствует некоторой области изначального выборочного пространства, ограниченной гиперплоскостями, параллельными осям признаков (во всех случаях кроме косых и сферических деревьев) 

В задаче классификации нам мало что остаётся делать, кроме как минимизировать эмпирический риск, беря моду всех меток среди тех что есть в нашей области (вполне логично). В задаче регрессии мы берем (и разберем это позже) усредненное значение меток $$ans_j = \dfrac{\sum\limits_{x_i\in area_j}y_i}{|x: x\in area_j|}$$
&nbsp;

### [[AIM ML#Content|Какое разбиение выбирать? Классификация]]

С предсказанием разобрались. *А как делать разбиение в каждом предикате*? Интуитивно понятно, что разбивать лучше разделяя объекты классов, а не оставляя их перемешанными

![[Pasted image 20230622003211.png]]

Для этого вводится *impurity* — мера неоднородности / чистоты множества. Пусть есть область $area$. Мы хотим побить её каким-то порогом на подобласти $area_L,\;area_R$. Тогда если $I(area)$ — мера частоты области, мы хотим максимизировать следующий функционал

$$A(area, \theta) = I(area) - \dfrac{|area_L|}{|area|}I(area_L) - \dfrac{|area_R|}{|area|}I(area_R)$$

Здесь нормирующие множители "штрафуют" за очень маленькое разбиение. То есть если мы нашли область, в которой большая чистота, но при этом сама область очень мала, то такое дерево слишком нерешительно и нам это не подходит (оно просто не научиться выявлять сложные паттерны)

На уровне интуиции формула ясна. Теперь нам нужно понять, каким должен быть функционал чистоты.

>[!example] Меры чистоты
>Пусть в области $area$ классы содержатся в пропорциях $p_1, p_2, \dots, p_k$
>- *Missclassification criteria / MC*: $I(area) = 1 - \max(p_i)$ 
>  (его максимальное значение это $1 - \dfrac{1}{k}$, что соответствует равномерному распределению и большой неоднородности, минимальное значение это 0, достигаемый в случае абсолютной однородности)
> #
>- *Энтропийная* мера (логично высчитывать однородность через самую известную меру неопределенности):  $I(area) = \sum\limits_{i}p_i\log_{2}(p_i)$
> #
> - *Джини*: $I(area) = \sum\limits_{i}p_i(1-p_i) = 1 - \sum\limits_{i}p_i^2$

>[!important] Сноска про критерии
>*Энтропийный критерий* явно выводится из желания минимизировать *логистическую функцию ошибки*
>
>*Критерий Джини*  явно выводится из минимизации *суммы квадратов отклонений / RSS*

Все эти меры при полностью неоднородном случае равны 0, а при полностью однородном близки к единице. Рассмотрим, чем же они отличаются с практической точки зрения. Для этого нарисуем зависимость каждого критерия от пропорций классов в случае с двумя классами

![[Pasted image 20230622140234.png]]

Вот как отличается функционал качества в модельной задаче при разных функциях однородности 

![[Pasted image 20230622113332.png]]

Как видим, MC ведет себя очень странно и не коррелирует с остальными колоколообразными графиками

>[!quote]- Обоснование энтропийного критерия расщепления
>Рассмотрим модельную задачу:
>
>![[Pasted image 20230622121752.png]]
>
>Совместная энтропия в этом случае равна $$H(x,y) = -\dfrac{3}{10}\log_{2}\dfrac{3}{10} -\dfrac{1}{10}\log_{2}\dfrac{1}{10} -\dfrac{1}{10}\log_{2}\dfrac{1}{10} -\dfrac{5}{10}\log_{2}\dfrac{5}{10}$$ 
>Однако если мы знаем, что $x = \text{"дождь"}$, возникает условная энтропия, записывающаяся проще $$H(y | x = \text{"дождь"}) = -\dfrac{3}{4}\log_{2}\dfrac{3}{4} -\dfrac{1}{4}\log_{2}\dfrac{1}{4}$$
>То же самое для заранее известного $x = \text{"сухо"}$:
>$$H(y | x = \text{"сухо"}) = -\dfrac{1}{6}\log_{2}\dfrac{1}{6} -\dfrac{5}{6}\log_{2}\dfrac{5}{6}$$
>Тогда *ожидаемая условная энтропия* — это среднее значение энтропии, когда мы проясним $x$:
>$$H(Y | X) = \dfrac{4}{10}H(y | x = \text{"дождь"}) + \dfrac{6}{10}H(y | x = \text{"сухо"})$$
>Отсюда логично вывести $Information gain$ — меру полезности определения признака (насколько в среднем изменится энтропия, если мы определим $x$):
>$$IG(Y|X) = {\color{goldenrod}H(X,Y)} - {\color{cyan}H(Y|X)}$$
>Так вот как раз эту штуку мы и считаем в формуле с энтропийным критерием:
>$$A(area, \theta) = {\color{goldenrod}I(area)} - {\color{cyan}\dfrac{|area_L|}{|area|}I(area_L) - \dfrac{|area_R|}{|area|}I(area_R)}$$
>Получается что мы максимизируем *information gain* , или *информационную выгоду*, что супер-мега-убер-логично

>[!important] Модификация impurity
>Часто так получается, что на начальных этапах найти хорошее разбиение не выходит. Тогда имеет смысл "переступить через себя" и разбить как-то, чтобы потом разбивать более точно. Для этого к impurity порой добавляют множитель, отвечающий за "рань" нашего разбиения — а именно нормированный размер области
>$$I(area) -> \dfrac{|area|}{n}I(area)$$

&nbsp;

### [[AIM ML#Content|Какое разбиение выбирать? Регрессия]]

Поступим аналогично, но теперь в качестве меры неоднородности логично взять дисперсию признака:
$$I(area) = var(y_i|y_i\in area)$$

&nbsp;

### [[AIM ML#Content|Какие разбиения проверять на практике?]]

Потенциально разбиений у нас континуум. Но не беда! Достаточно ведь проверять только одну точку между объектами. Все такие точки, дающие нам одинаковое разделение, дадут один функционал качества (можно заметить что график функционала кусочно постоянный)

>[!quote]- Обоснование критерия Джини
>Возьмем задачу регрессии, но в которой целевой признак бинарен $y_i \in \{0;1\}$
>Эмпирическая вероятность того, что метка = 1, есть $p = \dfrac{\sum y_i}{n}$
>Тогда нетрудно заметить, что дисперсия равна$$var(\{y_i\}) = p(1-p)$$ Что в точности есть *критерий Джини*! Получается, что он в каком-то смысле определяет дисперсию наших значений (если бы они были числовыми)

&nbsp;

### [[AIM ML#Content|Когда заканчивать?]]

- Слишком большая глубина / слишком много листов (глубина дерева очевидно увеличивает его сложность)
- Получилось слишком мало объектов в вершине (получили маленькую кучу, дальше её не делим)
- Получа*ется* слишком мало объектов в вершине (запрещаем разбивать так, чтобы получались маленькие кучи)
- В области все или почти все объекты одного класса
- Маленькое изменение *impurity*

&nbsp;

>[!warning] Проблема переобучения
>
>Как же нам бороться с переобучением?
>- Как и говорилось, глубокие деревья сложны и склонны к переобучению. Поэтому мы их ограничиваем
>- Другой способ подрезать деревья пост-фактум, или сделать *post-pruning*
> В этом случае используется модифицированный функционал качества, включающий в себя штраф за глубину дерева (регуляризатор). Называется функционал *MDL* (*minimum description length*)
>- Использовать не сами по себе, а в ансамбле (случайный лес). Тогда даже глубокие деревья, если их много разных, даёт хорошее качество
...
✅ Решающие деревья могут обучиться любой зависимости (если не ограничивать их длину)

✅ Можно использовать для признаков разных видов (категориальные / вещественные), также отлично работают с пропусками

✅ Можно сделать устойчивыми к выбросам (например штрафовать за сложность)

✅ Подходят для всех задач машинного обучения (классификация, регрессия. применяются в ансамблях)

✅ Достаточно быстро строятся

✅ Супер интерпретируемы, легко объяснить неспециалисту

✅ Устойчивы к дубликатам и зависимостям у объектов
...
❌ Не очень высокое качество и склонность к переобучению

❌ Неустойчивые

❌ Нет аналитической формулы

❌ Не используют расстояние до объектов
